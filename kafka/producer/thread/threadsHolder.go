...
func (t *ThreadsHolder) CountType(sig Status) int {
	...
	for i := range t.Threads {
		t.Mu[i].RLock()
		log.Printf("mutex %v locked for reading by arbitr\n", i)
		s := <-t.Threads[i].StatusChan
		if s == sig {
			cnt++
		}
		t.Mu[i].RUnlock()
		log.Printf("mutex %v unlocked by arbitr", i)
	}
	return cnt
}
...
func (t *ThreadsHolder) AppendBuffer(id int, msg string) {
	t.Mu[id].Lock()
	log.Printf("mutex %v locked for writing by goroutine %v\n", id, id)
	defer func() {
		t.Mu[id].Unlock()
		log.Printf("mutex %v unlocked by goroutine %v\n", id, id)
	}()
